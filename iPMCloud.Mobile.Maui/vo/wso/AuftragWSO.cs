...
namespace iPMCloud.Mobile
{
    ...
    public class AuftragWSO : ICloneable
    {
        ...
        public static Frame GetOrderCardView(AuftragWSO order, AppModel model, ICommand func)
        {
        ...
        public static Frame GetDisableOrderCardView(AuftragWSO order, AppModel model)
        {
            var _prio = CalcOverdue(order, model);
            var imageL = new Image
            {
        ...
        public static Frame GetOrderAgainCardView(AuftragWSO order, AppModel model)
        {
            var _prio = CalcOverdue(order, model);
            var imageL = new Image
            {
            ...
            var h = new StackLayout()
            {
                ...
                Orientation = StackOrientation.Horizontal,
                HorizontalOptions = LayoutOptions.FillAndExpand,
                BackgroundColor = Color.FromHex("#33042d53"),
            };
            var v = new StackLayout()
            {
                Padding = new Thickness(0),
            ...
            var mainFrame = new Frame()
            {
                ...
                Margin = new Thickness(0, 0, 0, 0),
                HorizontalOptions = LayoutOptions.FillAndExpand,
                BackgroundColor = Color.FromHex("#33041d43"),
                Content = h,
                CornerRadius = 0,
                HasShadow = true,
                IsClippedToBounds = true,
        ...
        public static StackLayout GetOrderTodoCardViewOnlyKat(AuftragWSO order, AppModel model, double _prio, bool onlyText)
        {
            ...
            var h = new StackLayout()
            {
                ...
                Orientation = StackOrientation.Horizontal,
                HorizontalOptions = LayoutOptions.FillAndExpand,
                BackgroundColor = onlyText ? Color.FromHex("#aa04532d") : Color.FromHex("#aa042d53"),
            };
        ...
        public static StackLayout GetOrderTodoWarning(double count, AppModel model, double countMax = double.NaN)
        {
            ...
            if (!double.IsNaN(countMax))
            {
                ...
                badgeMax = new Frame
                {
                    BackgroundColor = Color.FromHex(countMax < 0 ? "#ff0000" : (countMax < 1 ? "#ffcc00" : "#009900")),
                    IsClippedToBounds = true,
                    HasShadow = true,
                    HorizontalOptions = LayoutOptions.End,
                    VerticalOptions = LayoutOptions.Center,
        ...
        public static Frame GetTodoCountWarningSmall(double count)
        {
            var badge = new Frame
            {
                BackgroundColor = Color.FromHex(count < 0 ? "#ff0000" : (count < 1 ? "#ffcc00" : "#009900")),
