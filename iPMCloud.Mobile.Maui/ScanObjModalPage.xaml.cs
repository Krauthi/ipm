    ...
    public partial class ScanObjModalPage : ContentPage
    {
        ...
        protected override void OnAppearing()
        {
            ...
            MainThread.BeginInvokeOnMainThread(async () =>
            {
                try
                {
                    AppModel.Logger.Info("[ScanObjModalPage] Android preview workaround: delay 120ms before reinit.");
                    ...
                    AppModel.Logger.Info($"[ScanObjModalPage] Android preview workaround: camera toggle {originalCamera} -> {toggledCamera}.");
                    ReaderView.CameraLocation = toggledCamera;
                    await Task.Delay(120, token);
                    AppModel.Logger.Info($"[ScanObjModalPage] Android preview workaround: camera toggle {toggledCamera} -> {originalCamera}.");
